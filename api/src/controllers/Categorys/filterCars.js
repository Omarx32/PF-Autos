    ...
    try {
        ...
        for (let i = 0; i < cars.length; i++) {
            for (let j = 0; j < cars[i].Categories.length; j++) {
                cars[i].Categories[j].dataValues.name=diacriticless( cars[i].Categories[j].dataValues.name.toLowerCase())                
            }
        }
        ...
        for(let i = 0; i < cars.length; i++){
            if(Array.isArray(cars[i].Categories)){
                for(let j = 0; j < cars[i].Categories.length; j++){
                    if(cars[i].Categories[j].dataValues.name===catcar && !carsFilt.includes(cars[i])){
