...
async function createProduct(req, res) {
    ...
    for (const productInput of productsInput) {
      const {name, brand, description, price, stock, maker, model, color, kilometraje, direccion, category, image,} = productInput;

      if (!name ||!brand ||!description ||!price ||!stock ||!maker ||!model ||!color ||!kilometraje || !direccion ||!image) { 
         throw new Error("Missing required data");}
    
        const imageUrls = [];
        for (const imageData of productInput.image){
